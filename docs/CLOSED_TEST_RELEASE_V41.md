# Smart Cleaner v41 - Play closed-test handoff
...
- `SmartCleaner-PLAY-CLOSED-TEST-AAB-v41` contains only `SmartCleaner-ClosedTest-v41.aab`. This is the only file to upload to Play Console.
- `SmartCleaner-CLOSED-TEST-APK-v41` contains `SmartCleaner-ClosedTest-v41.apk` for direct device installation and smoke testing.
- `SmartCleaner-ClosedTest-Diagnostics-v41` contains mapping, lint, signature/binary audit, hashes, and publication-ready legal Markdown with `SUPPORT_EMAIL` resolved.
...
5. Upload only `SmartCleaner-ClosedTest-v41.aab` to the intended closed-testing track.
