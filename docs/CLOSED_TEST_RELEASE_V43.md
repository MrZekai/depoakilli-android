# Smart Cleaner v43 - Play closed-test handoff
...
- `SmartCleaner-PLAY-CLOSED-TEST-AAB-v43` contains only `SmartCleaner-ClosedTest-v43.aab`. This is the only file to upload to Play Console.
- `SmartCleaner-CLOSED-TEST-APK-v43` contains `SmartCleaner-ClosedTest-v43.apk` for direct device installation and smoke testing.
- `SmartCleaner-ClosedTest-Diagnostics-v43` contains mapping, lint, signature/binary audit, hashes, and publication-ready legal Markdown with `SUPPORT_EMAIL` resolved.
...
5. Upload only `SmartCleaner-ClosedTest-v43.aab` to the intended closed-testing track.
