# Smart Cleaner v40 - Play closed-test handoff
...
- `SmartCleaner-PLAY-CLOSED-TEST-AAB-v40` contains only `SmartCleaner-ClosedTest-v40.aab`. This is the only file to upload to Play Console.
- `SmartCleaner-CLOSED-TEST-APK-v40` contains `SmartCleaner-ClosedTest-v40.apk` for direct device installation and smoke testing.
- `SmartCleaner-ClosedTest-Diagnostics-v40` contains mapping, lint, signature/binary audit, hashes, and publication-ready legal Markdown with `SUPPORT_EMAIL` resolved.
...
5. Upload only `SmartCleaner-ClosedTest-v40.aab` to the intended closed-testing track.
