# Smart Cleaner v42 - Play closed-test handoff
...
- `SmartCleaner-PLAY-CLOSED-TEST-AAB-v42` contains only `SmartCleaner-ClosedTest-v42.aab`. This is the only file to upload to Play Console.
- `SmartCleaner-CLOSED-TEST-APK-v42` contains `SmartCleaner-ClosedTest-v42.apk` for direct device installation and smoke testing.
- `SmartCleaner-ClosedTest-Diagnostics-v42` contains mapping, lint, signature/binary audit, hashes, and publication-ready legal Markdown with `SUPPORT_EMAIL` resolved.
...
5. Upload only `SmartCleaner-ClosedTest-v42.aab` to the intended closed-testing track.
